...
- When promoting F# functions from `internal` to public, the `internal` keyword must be removed from BOTH the `.fsi` signature file AND the `.fs` implementation file. The compiler requires implementation accessibility to match or exceed signature accessibility (FS0034). [Source: specs/005-expose-session-state]
- PhysicsClient Session module public API expanded to 7 functions: `connect`, `disconnect`, `reconnect`, `isConnected`, `bodyRegistry`, `latestState`, `lastStateUpdate`. Surface area baseline updated. [Source: specs/005-expose-session-state]
