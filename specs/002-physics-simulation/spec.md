...
- The simulation uses BepuFSharp, an idiomatic F# wrapper for the BepuPhysics2 rigid body engine, consumed via local NuGet package. Only basic features (body creation, force/impulse application, gravity) are used. Advanced features (constraints, contact events, raycasting) are available for future specs.
