...
A developer notices that 6 integration test files contain only 1 test each (DiagnosticsIntegrationTests, ComparisonIntegrationTests, StaticBodyTests, StressTestIntegrationTests, RestartIntegrationTests, CommandAuditStreamTests). They merge these into logically related, feature-focused files to reduce file proliferation and improve discoverability.
...
1. **Given** 6 integration test files have 1 test each, **When** they are consolidated into related feature files, **Then** the number of integration test files decreases while all tests still pass
